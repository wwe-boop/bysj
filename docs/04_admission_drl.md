...
命名对齐：文中 \(A^{pos}\) 在接口/脚本中统一为字段名 `Apos`；定位权重对应配置键 `lambda_pos`。

---

## 4.8 差异与假设（新增）

- 工程化指标 `Apos`：将可见波束/协作卫星/CRLB 阈值组合为可用性分数并并入奖励，这区别于参考中仅报告 CRLB/GDOP 的做法；`Apos` 作为策略层抽象，便于权重化与接口对齐。
- Beam Hint 的作用位阶：本文将 Beam Hint 既用于状态特征也作为执行侧软约束/触发重分配的信号，超出通常仅用于定位估计的范围。
- 细化假设：
  - 噪声/偏差：观测噪声协方差 `R` 采用场景化设定，未在本章展开 NLOS、同步误差、硬件偏差的细粒度建模；其影响在第6章以敏感性实验体现。
  - 估计器：不限定 EKF/UKF/粒子滤波等具体实现，仅以 FIM/CRLB 近似指导奖励与约束；实现细节交由定位模块。
  - 理论保证：PPO 收敛与全局最优性不做理论保证；稳定性由执行层（李雅普诺夫/队列）与工程约束（冷却/惩罚）共同保障。

> 目的：明确本章相对参考工作的工程化扩展与边界假设，避免与纯理论推导产生混淆。
